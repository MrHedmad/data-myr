...
def myr_check(path: Path) -> None:
    log.debug(f"Invoked `myr_check` with {path}")
    raise NotImplementedError()

def myr_freeze(input_path: Path, output_path: Path) -> None:
    log.debug(f"Invoked `myr_freeze` with input - {input_path} - and output - {output_path}")
    raise NotImplementedError()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(prog = "myr", description = "Package your data in a FAIR way.")
    ...
    create_cmd.add_argument("path", default=".", type=Path, help = "destination folder", nargs = "?")
    create_cmd.add_argument("-f", "--force", action="store_true", help = "force creation, ignoring existing files")

    # `myr check` - checks a myr bundle for validity
    check_cmd = subparsers.add_parser("check", help = "check a myr bundle for validity.")
    check_cmd.add_argument("path", default=".", type=Path, help = "where to check", nargs = "?")

    # `myr freeze` - freezes a myr bundle
    freeze_cmd = subparsers.add_parser("freeze", help = "freeze a myr bundle.")
    freeze_cmd.add_argument("input_path", default=".", type=Path, help="bundle to freeze", nargs = "?")
    freeze_cmd.add_argument("--output", default=None, type=Path, help="output frozen bundle filename")
    ...
    match args.command:
        ...
        case "check":
            myr_check(args.path.expanduser().resolve())
        case "freeze":
            input_path = args.input_path.expanduser().resolve()
            outfile = input_path.parent / f"{input_path.stem}.tar.gz" if args.output is None else args.output
